...
capa: cliente
estado: en-progreso
interfaz: createCamera, buildVideoConstraints, assertGetUserMedia, createResultBus, createStats, createThrottle, createRenderer, createHud, faceResultToDetectionPayload, analyzeFace (expresiones+pose de cabeza), inference.worker
...
verificado_por: apps/client/test/ (25 tests; mapping validado contra el contrato, math de pose de cabeza con matrices conocidas), node --check, curl del dev server (http y https con SAN de LAN, headers COOP/COEP)
---
...
**Análisis (primer módulo de dominio):** `analysis.js` consume la detección del **contrato** y deriva **expresiones** (sonrisa, boca abierta, parpadeo, cejas — desde los blendshapes) y **pose de cabeza** (yaw/pitch/roll desde la matriz 4×4), mostradas en el HUD. La extracción de ángulos de Euler está testeada con matrices de rotación conocidas; la convención de ejes/columna se confirma visualmente en hardware (ajuste de una línea si algún eje sale invertido).
